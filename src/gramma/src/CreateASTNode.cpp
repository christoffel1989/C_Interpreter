...
using TkResult = std::tuple<Token, std::string>;
...
{
	//读取分号
	return expectToken(input, "error(bad syntax): not ;!\n", TokenType::End)
			.and_then([](std::tuple<Token, std::string> result) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
				{
					//创建父节点
					...
					return std::make_tuple(parent, std::get<1>(result));
				});
}
...
auto createDeRefASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取解引用符号(*)
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss a *!", TokenType::Mul));

	//读取自定义变量或者左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): dereference(*) must be followed by variable or a (arithmetic expression)!", TokenType::Mul));
	...
	{
		//解析一个算数表达式
		TRY_PARSE(child, input, createArithmeticASTNode(input));

		//再解析一个右括号
		TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss a )!", TokenType::Rp));
	}
	parent->childs.push_back(child);

	return std::make_tuple(parent, input);
}
...
auto createFactorASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	if (isoneof(tk.type, TokenType::Number, TokenType::Plus, TokenType::Minus, TokenType::Not, TokenType::And, TokenType::PlusPlus, TokenType::MinusMinus))
	{
		...
		if (tk.type != TokenType::Number)
		{
			...
			TRY_PARSE(child, input, createFactorASTNode(input));
			parent->childs.push_back(child);
		}
	}
	//*变量解引用
	...
	{
		//把*号补回去再解析
		TRY_PARSE(parent, input, createDeRefASTNode("*" + input));
	...
	{
		//括号中的表达式
		TRY_PARSE(parent, input, createExpressionASTNode(input));
	...
	else if (tk.type == TokenType::PrimitiveSymbol)
	{
		...
		if (std::holds_alternative<std::function<double(double)>>(primitive))
		{
			...
			//读取右小括号
			TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): not enough arguments for function call or function call miss a )!", TokenType::Rp));
	...
	else if (tk.type == TokenType::UserSymbol)
	{
		...
		if (tk.type == TokenType::Lp)
		{
			...
			if (tk.type != TokenType::Rp)
			{
				...
				if (tk.type != TokenType::Rp)
				{
					return std::unexpected("error(bad syntax): not enough arguments for function call or function call miss a )!\n");
				}
			}
			//无输入参数 0元函数
			else
		...
		else if (tk.type == TokenType::LBracket)
		{
			...
			//创建算术节点
			TRY_PARSE(child2, input, createArithmeticASTNode(res1));

			//再读取一个右中括号
			TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss a ]!", TokenType::RBracket));
	...
	{
		return std::unexpected("error(bad syntax): need a number or a (!\n");
	}
...
auto createTermASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//获得第一个项的节点并存储在父节点位置
	if (auto result = createFactorASTNode(input))
	{
		std::tie(parent, input) = result.value();
	}
	else
	{
		return result;
...
auto createArithmeticASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	std::shared_ptr<ASTNode> parent, child1, child2;
	//获得第一个项的节点并存储在父节点位置
	TRY_PARSE(parent, input, createTermASTNode(input));
...
auto createRelationASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	std::shared_ptr<ASTNode> parent, child1, child2;
	//获得第一个项的节点并存储在父节点位置
	TRY_PARSE(parent, input, createArithmeticASTNode(input));
...
auto createLogicASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	std::shared_ptr<ASTNode> parent, child1, child2;
	//获得第一个项的节点并存储在父节点位置
	TRY_PARSE(parent, input, createRelationASTNode(input));
	...
	while (true)
	{
		...
		if (isoneof(op.type, TokenType::AndAnd, TokenType::OrOr))
		{
			...
			child1 = parent;
			//获取子节点2
			TRY_PARSE(child2, input, createRelationASTNode(str));
			//创建父节点
			parent = std::make_shared<ASTNode>();
...
auto createDefArrayASTNode(std::shared_ptr<ASTNode> node, std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	Token tk;
	//读取左中括号
	TRY_PARSE(tk, input, expectToken(input, "error(Def array): illegal syntax for defining array!", TokenType::LBracket));
	//读取数字或者右中括号
	TRY_PARSE(tk, input, expectToken(input, "error(Def array): illegal syntax for defining array!", TokenType::Number, TokenType::RBracket));
	...
	if (tk.type == TokenType::Number)
	{
		...
		vnodes.resize(N);
		//读取右中括号
		TRY_PARSE(tk, input, expectToken(input, "error(Def var): illegal syntax for defining array!", TokenType::RBracket));
		TRY_PARSE_IGNORE(input, expectToken(input, "error(Def var): illegal syntax for defining array!", TokenType::RBracket));
		//读取赋值号或者分号或者空
		TRY_PARSE_AUTO(tknext, res, expectToken(input, "error(Def array): illegal syntax for defining array!", TokenType::Assign, TokenType::End, TokenType::Empty));
		if (tknext.type == TokenType::Assign)
		{
			//读取左大括号
			TRY_PARSE(tk, input, expectToken(res, "error(Def array): need a {!", TokenType::LBrace));
			//读取数组初始化的各个元素值
			for (decltype(N) i = 0; i < N; i++)
			{
				//获取第i个元素的语法节点
				TRY_PARSE(vnodes[i], input, createArithmeticASTNode(input));
				//如果不是最后一个元素则再读取一个逗号
				if (i != N - 1)
				{
					//读取左中括号
					TRY_PARSE_IGNORE(input, expectToken(input, "error(Def array): array elment num mismatch!", TokenType::Comma));
				}
			}
			//读取右大括号
			TRY_PARSE(tk, input, expectToken(input, "error(Def array): array elment num mismatch!", TokenType::RBrace));
		}
		//默认初始化
		else
		{
	...
	{
		//读取赋值号
		TRY_PARSE_AUTO(tknext, res, expectToken(input, "error(Def array): need a =!", TokenType::Assign));
		//读取左大括号
		TRY_PARSE(tk, input, expectToken(res, "error(Def array): need a {!", TokenType::LBrace));
		do
		{
		...
		{
			return std::unexpected("error(Def array): need a }!\n");
		}
	}
...
auto createExpressionASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	if (tk.type == TokenType::DefVar)
	{
		...
		TRY_PARSE(tk, input, expectToken(str, "error(Def var or pointer): need ( or a symbol to reprensent variable name !", TokenType::UserSymbol, TokenType::Mul));
		...
		if (tk.type == TokenType::UserSymbol)
		{
			...
			TRY_PARSE_AUTO(tknext, res, expectToken(input, "error(Def var): illegal syntax for defining varible!", TokenType::Assign, TokenType::LBracket, TokenType::End, TokenType::Empty));

			//读出来的是赋值号
			if (tknext.type == TokenType::Assign)
			{
				//创建代表赋值=号右边表达式的语句
				TRY_PARSE(child, input, createExpressionASTNode(res));
				//child与parent连接
				parent->childs.push_back(child);
			}
			//读出来的是中括号 则定义的是数组
			...
			{
				//创建定义数组的语法节点
				TRY_PARSE(parent, input, createDefArrayASTNode(child, input));
			}
			//读出来的是分号或者空
			else
			{
		...
		{
			//读取定义的变量名
			TRY_PARSE(tk, input, expectToken(input, "error(Def pointer): need a symbol to reprensent pointer name!", TokenType::UserSymbol));
			...
			//读取赋值符号或者中括号或者分号或者啥也没有
			TRY_PARSE_AUTO(tknext, res, expectToken(input, "error(Def var): illegal syntax for defining pointer!", TokenType::Assign, TokenType::LBracket, TokenType::End, TokenType::Empty));

			//读出来的是赋值号
			if (tknext.type == TokenType::Assign)
			{
				//创建代表赋值=号右边算表达式语句
				TRY_PARSE(child, input, createArithmeticASTNode(res));
				//child与parent连接
				parent->childs.push_back(child);
			}
			//读出来的是中括号 则定义的是数组
			...
			{
				TRY_PARSE(parent, input, createDefArrayASTNode(parent, input));
			}
			//读出来的是分号或者空
			else
			{
	...
	else if (tk.type == TokenType::UserSymbol)
	{
		...
		if (isoneof(tktemp.type, TokenType::Assign, TokenType::SelfPlus, TokenType::SelfMinus, TokenType::SelfMul, TokenType::SelfDiv))
		{
			...
			parent->childs.push_back(child);
			//创建代表赋值=号右边表达式的语句
			TRY_PARSE(child, input, createExpressionASTNode(str));
			//child与parent连接
			parent->childs.push_back(child);
		}
		else
		{
			//其他情况下用逻辑运算语句解析
			TRY_PARSE(parent, input, createLogicASTNode(input));
		}
	}
	//如果是Mul(即指针解引用)
	else if (tk.type == TokenType::Mul)
...
auto createBlockASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取左大括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss a {!", TokenType::LBrace));
	...
	do
	{
		...
		if (isnoneof(child->tk.type, TokenType::Block, TokenType::If, TokenType::While, TokenType::Do, TokenType::For))
		{
			...
			{
				return std::unexpected("error(bad syntax): miss a ;!\n");
			}
			else
			{
				input = str;
...
auto createIfASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字if
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword if!", TokenType::If));
	...
	//读取左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): if condition need a (!", TokenType::Lp));

	//读取条件
	decltype(parent) ifcondition;
	TRY_PARSE(ifcondition, input, createExpressionASTNode(input));

	//添加为parent的第1个儿子
	parent->childs.push_back(ifcondition);

	//读取右括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): if condition need a )!", TokenType::Rp));

	//读取if成立的block
	decltype(parent) ifblock;
	TRY_PARSE(ifblock, input, createBlockASTNode(input));
	...
	if (tk.type == TokenType::ElseIf)
	{
		...
		TRY_PARSE(elseifnode, input, createElseIfASTNode(input));
		//添加为parent的第3个儿子
	...
	else if (tk.type == TokenType::Else)
	{
		...
		TRY_PARSE(elseblock, input, createBlockASTNode(input));
		//添加为parent的第3个儿子
...
auto createElseIfASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字elseif
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword elseif!", TokenType::ElseIf));
	...
	//读取左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): elseif condition need a (!", TokenType::Lp));
	...
	//读取右括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): elseif condition need a )!", TokenType::Rp));
	...
	{
		decltype(parent) elseifnode;
		TRY_PARSE(elseifnode, input, createElseIfASTNode(str));
		//添加为parent的第3个儿子
		parent->childs.push_back(elseifnode);
	}
	else if (tk.type == TokenType::Else)
	{
		decltype(parent) elseblock;
		TRY_PARSE(elseblock, input, createBlockASTNode(str));
		//添加为parent的第3个儿子
		parent->childs.push_back(elseblock);
	}
...
auto createWhileASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字while
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword while!", TokenType::While));
	...
	//读取左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): while loop condition need a (!", TokenType::Lp));

	//循环条件
	decltype(parent) loopcondition;
	TRY_PARSE(loopcondition, input, createExpressionASTNode(input));

	//添加为parent的第1个儿子
	parent->childs.push_back(loopcondition);

	//读取右括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): while loop condition need a )", TokenType::Rp));

	//读取while的循环体
	decltype(parent) loopblock;
	TRY_PARSE(loopblock, input, createBlockASTNode(input));
...
auto createDoWhileASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字do
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword do!", TokenType::Do));
	...
	//读取左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): do while loop condition need a (!", TokenType::Lp));
	...
	//读取右括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): do while loop condition need a )", TokenType::Rp));

	//读取分号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss a ;", TokenType::End));
...
auto createForASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字for
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword for!", TokenType::For));
	...
	//读取左括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): for loop condition need a (!", TokenType::Lp));

	//循环起始
	decltype(parent) start;
	TRY_PARSE(start, input, createExpressionASTNode(input));

	//添加为parent的第1个儿子
	parent->childs.push_back(start);

	//读取分号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): for loop need a ;!", TokenType::End));

	//循环终止条件
	decltype(parent) end;
	TRY_PARSE(end, input, createExpressionASTNode(input));
	...
	TRY_PARSE(increment, input, createExpressionASTNode(input));

	//添加为parent的第3个儿子
	parent->childs.push_back(increment);

	//读取右括号
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): for loop condition need a )!", TokenType::Rp));
...
auto createBreakorContinueASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	TRY_PARSE(tk, input, expectToken(input, "error(bad syntax): miss keyword break or continue!", TokenType::Break, TokenType::Continue));
...
{
	//读取关键字return
	TRY_PARSE_IGNORE(input, expectToken(input, "error(bad syntax): miss keyword return!", TokenType::Return));
...
auto createDefProcASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	//读取关键字proc
	TRY_PARSE_IGNORE(input, expectToken(input, "error(Def proc): miss keyword proc!", TokenType::DefProc));
	//读取变量名
	TRY_PARSE(tkfun, input, expectToken(input, "error(Def proc): miss function name!", TokenType::UserSymbol));
	//读取左括号
	TRY_PARSE_IGNORE(input, expectToken(input, "error(Def proc): miss a (", TokenType::Lp));
	...
	else
	{
		...
		{
			//读取函数输入参数名
			TRY_PARSE(tkarg, input, expectToken(input, "need a symbo for function para name!", TokenType::UserSymbol));
		...
		if (tk.type != TokenType::Rp)
		{
			return std::unexpected("error(define proc): miss a )!\n");
		}
	}

	//读取赋值号
	TRY_PARSE_IGNORE(input, expectToken(input, "error(define proc): need a =!", TokenType::Assign));
	...
	TRY_PARSE(body, input, createBlockASTNode(input));
...
//构建一个映射表
static std::unordered_map<TokenType, std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>(*)(std::string)> ASTNodeMap = 
{
...
auto createStatementASTNode(std::string input) -> std::expected<std::tuple<std::shared_ptr<ASTNode>, std::string>, std::string>
{
	...
	{
		TRY_PARSE(parent, input, ASTNodeMap[tk.type](input))
	}
	else
	{
		//其他情况下用表达式语句解析
